	...
	*/
	 ...
	 */
	 function uploads($files=array(),$type=array(),$upfile='',$backPage=''){
		...
		if(strlen($upfile)<=0){
			$upfile=UPLOAD_PATH;//UPLOAD_PATH--这个常量在include/config.php里定义的
		}
			...
			if($files_error[$i]===0){//判断是否上传成功
				if(is_uploaded_file($files_tmp_name[$i])){//判断是不是post提交
                    if(strpos($files_name[$i],'.')!==false){//判断文件里是否有"."
					   ...
					   if(in_array($f_name,$type)){
						   ...
						   $f_name1=FILE_PATH.$f_name;//FILE_PATH--这个常量在include/config.php里定义的
				...
				*/
			    ...
			    switch ($files_error[$i]){
				   case 1:
                         echo "<script>alert('上传第".($i+1)."个文件发生错误，原因是：上传的文件超过了服务器配置文件限制的大小');window.location.href='".$backPage."';</script>";
			             exit;
				   case 2:
                         echo "<script>alert('上传第".($i+1)."个文件发生错误，原因是：上传的文件超过了页面表单限制的大小');window.location.href='".$backPage."';</script>";
			             exit;
				   case 3:
                         echo "<script>alert('上传第".($i+1)."个文件发生错误，原因是：文件只有部分被上传');window.location.href='".$backPage."';</script>";
			             exit;
				   case 4:
                         echo "<script>alert('上传第".($i+1)."个文件发生错误，原因是：没有文件被上传');window.location.href='".$backPage."';</script>";
			             exit;
				   case 6:
                         echo "<script>alert('上传第".($i+1)."个文件发生错误，原因是：找不到临时文件目录');window.location.href='".$backPage."';</script>";
			             exit;
				   case 7:
                         echo "<script>alert('上传第".($i+1)."个文件发生错误，原因是：写入失败');window.location.href='".$backPage."';</script>";
			             exit;
				   default:
                         echo "<script>alert('上传第".($i+1)."个文件发生未知错误');window.location.href='".$backPage."';</script>";
			             exit;
			    }
			}
		}
	 ...
	 */
	 function upload($file=array(),$type=array(),$upfile='',$backPage=''){
		...
		if($file['error']==0){//判断是否上传成功
            if(is_uploaded_file($file['tmp_name'])){//判断是不是post提交
				...
				if($isExists!==false){
					...
					if(in_array($filename,$type)){
						...
						$f_name=FILE_PATH.$filename;//FILE_PATH--这个常量在include/config.php里定义的
						$flag=move_uploaded_file($file['tmp_name'],$f_name);
						if($flag){
							   $back=$filename;
						 }else{
			...
			switch($file['error']){
				case 1:
                      echo "<script>alert('上传文件发生错误，原因是：上传的文件超过了服务器配置文件限制的大小');window.location.href='".$backPage."';</script>";
			          exit;
				case 2:
                      echo "<script>alert('上传文件发生错误，原因是：上传的文件超过了页面表单限制的大小');window.location.href='".$backPage."';</script>";
			          exit;
				case 3:
                      echo "<script>alert('上传文件发生错误，原因是：文件只有部分被上传');window.location.href='".$backPage."';</script>";
			          exit;
				case 4:
                      echo "<script>alert('上传文件发生错误，原因是：没有文件被上传');window.location.href='".$backPage."';</script>";
			          exit;
				case 6:
                      echo "<script>alert('上传文件发生错误，原因是：找不到临时文件目录');window.location.href='".$backPage."';</script>";
			          exit;
				case 7:
                      echo "<script>alert('上传文件发生错误，原因是：写入失败');window.location.href='".$backPage."';</script>";
			          exit;
				default:
                         echo "<script>alert('上传文件发生未知错误');window.location.href='".$backPage."';</script>";
			             exit;
			}
		}
		return $back;
	 ...
	 */
	 function down($path='',$backPage=''){
		 ...
		 if(strlen($path)<=0){
		    ...
		    $path=FILE_PATH.$path;//FILE_PATH--这个常量在include/config.php里定义的
		 }
